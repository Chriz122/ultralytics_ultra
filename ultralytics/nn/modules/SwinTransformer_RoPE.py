# --------------------------------------------------------
# Swin Transformer with 2D RoPE (Rotary Position Embedding)
...
def compute_cis(freqs: torch.Tensor, t_x: torch.Tensor, t_y: torch.Tensor):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        # 強制轉型為 float32 (FP32) 來避開 torch.polar 不支援 FP16 的問題
        freqs = freqs.float()
        t_x = t_x.float()
        t_y = t_y.float()
    ...
    def __init__(self, pretrain_img_size=224, patch_size=4, in_chans=3, embed_dim=96,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                rope_theta=rope_theta, rope_mixed=rope_mixed, use_rpb=use_rpb) # Pass RoPE params
        ...
        # Initial forward pass to compute width_list dynamically 
...
# Default Helper Instantiations
def SwinTransformer_RoPE_Tiny(weights=''):
    model = SwinTransformer(depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24], rope_theta=10.0, rope_mixed=True, use_rpb=False)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['model']))
    return model

def SwinTransformer_RoPE_Tiny_c24(weights=''):
    model = SwinTransformer(pretrain_img_size=256, window_size=8, depths=[2, 2, 6, 2], num_heads=[4, 8, 16, 32], rope_theta=10.0, rope_mixed=True, use_rpb=False)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['model']))
    return model

def SwinTransformer_RoPE_Small(weights=''):
    model = SwinTransformer(depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24], rope_theta=10.0, rope_mixed=True, use_rpb=False)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['model']))
    return model

def SwinTransformer_RoPE_Base(weights=''):
    model = SwinTransformer(depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32], rope_theta=10.0, rope_mixed=True, use_rpb=False)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['model']))
    return model

def SwinTransformer_RoPE_Large(weights=''):
    model = SwinTransformer(depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48], rope_theta=10.0, rope_mixed=True, use_rpb=False)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['model']))
    return model
...
if __name__ == '__main__':
    ...
    model = SwinTransformer(rope_theta=10.0, rope_mixed=True, use_rpb=False).to(device)
