...
class AFSSSampler(torch.utils.data.Sampler):
    """單卡訓練用的 AFSS 動態 Sampler"""
    def __init__(self, dataset, shuffle=True):
        self.dataset = dataset
        self.shuffle = shuffle
        self.active_indices = list(range(len(dataset)))
        self.epoch = 0
    ...
    def __iter__(self):
        if self.shuffle:
            g = torch.Generator()
            g.manual_seed(self.epoch)
            indices = [self.active_indices[i] for i in torch.randperm(len(self.active_indices), generator=g).tolist()]
        else:
...
class AFSSDistributedSampler(torch.utils.data.DistributedSampler):
    """多卡 DDP 訓練用的 AFSS 動態 Sampler"""
    ...
    def set_indices(self, indices):
        self.active_indices = indices
        self.num_samples = math.ceil(len(self.active_indices) / self.num_replicas)
    ...
    def __iter__(self):
        if self.shuffle:
            ...
            indices = self.active_indices.copy()
        # Padding to make it evenly divisible
...
def build_dataloader(
    ...
    # === 新增/修改：套用 AFSS Sampler ===
    # 若為 train mode 則套用 AFSS Sampler
    if getattr(dataset, "augment", False):  # train dataset typically has augment=True
        if rank == -1:
            sampler = AFSSSampler(dataset, shuffle=shuffle)
        else:
            sampler = AFSSDistributedSampler(dataset, num_replicas=nd, rank=rank, shuffle=shuffle)
    else:
        # 原本驗證集的邏輯
        sampler = (
            None
