...
from natten.functional import na2d # 替換為新的 na2d
...
def selective_scan_state_fn(u, delta, A, B, D=None, z=None, delta_bias=None, delta_softplus=False, return_last_state=False):
    # 【CPU Bypass 防護】確保 YOLO 初始化計算 Stride 時能成功進行 Shape Inference
    if not u.is_cuda:
        B_batch, D_dim, L = u.shape
        N = A.shape[1]
        dummy_out = torch.zeros(B_batch, D_dim, N, L, device=u.device, dtype=u.dtype)
        if return_last_state:
            dummy_state = torch.zeros(B_batch, D_dim, N, device=u.device, dtype=u.dtype)
...
class A2SSM(nn.Module):
    def __init__(
        self, d_model, d_state=1, d_conv=5, expand=1,
        dt_rank="auto", dt_min=0.001, dt_max=0.1, dt_init="random",
        dt_scale=1.0, dt_init_floor=1e-4, dropout=0,
        device=None, dtype=None, num_heads=1, **kwargs,
    ):
    ...
    def forward_ssm(self, x, state_params=None):
        ...
        if state_params is not None:
            q1, k1, q2, k2, ws, dil1, dil2 = state_params
            n_heads_half = q1.shape[3] # [b, h, w, n, c]
            
            # 轉換為 na2d 可接收的 [b, h, w, n, d]
            state = rearrange(state, "b d s (h w) -> b (d s) (h w)", h=H, w=W)
            state = rearrange(state, "b (m n d) (h w) -> m b h w n d", m=2, n=n_heads_half, h=H, w=W).contiguous()
            
            # 使用 na2d 來同時進行 QK 和 AV 運算
            # 【CPU Bypass 防護】
            if not q1.is_cuda:
                state1 = torch.zeros_like(state[0])
                state2 = torch.zeros_like(state[1])
            else:
                state1 = na2d(q1, k1, state[0], kernel_size=toodd(ws), dilation=dil1)
                state2 = na2d(q2, k2, state[1], kernel_size=toodd(ws), dilation=dil2)
            
            state1 = rearrange(state1, "b h w n d -> b n h w d")
            state2 = rearrange(state2, "b h w n d -> b n h w d")
...
class Attention(nn.Module):
    def __init__(self, embed_dim=64, num_heads=2, window_size=7, window_dilation=1, global_mode=False, image_size=None, use_rpb=False, sr_ratio=1):
        super().__init__()
        ...
        # NATTEN 新版 na2d API 未暴露 rpb，因此局部 attention 不使用 RPB 偏置
        # 僅為 global_mode 保留 RPB
        if use_rpb:
            rpb_list = []
            if global_mode: 
    ...
    def forward(self, x, pos_enc):
        ...
        if not self.global_mode:
            # 將形狀轉換為 na2d 要求的: [batch, X, Y, heads, head_dim]
            q1_bhwnc = rearrange(q1, 'b n h w c -> b h w n c').contiguous()
            k1_bhwnc = rearrange(k1, 'b n h w c -> b h w n c').contiguous()
            v1_bhwnc = rearrange(v1, 'b n h w c -> b h w n c').contiguous()

            # 【CPU Bypass 防護】
            if not q1_bhwnc.is_cuda:
                v1_out = torch.zeros_like(v1_bhwnc)
            else:
                v1_out = na2d(q1_bhwnc, k1_bhwnc, v1_bhwnc, kernel_size=toodd(self.window_size), dilation=self.window_dilation)
            v1_out = rearrange(v1_out, 'b h w n c -> b n h w c')

            q2_bhwnc = rearrange(q2, 'b n h w c -> b h w n c').contiguous()
            k2_bhwnc = rearrange(k2, 'b n h w c -> b h w n c').contiguous()
            v2_in = v1 + v2
            v2_bhwnc = rearrange(v2_in, 'b n h w c -> b h w n c').contiguous()

            # 【CPU Bypass 防護】
            if not q2_bhwnc.is_cuda:
                v2_out = torch.zeros_like(v2_bhwnc)
            else:
                v2_out = na2d(q2_bhwnc, k2_bhwnc, v2_bhwnc, kernel_size=toodd(self.window_size), dilation=dilation)
            v2_out = rearrange(v2_out, 'b h w n c -> b n h w c')

            x = torch.cat([v1_out, v2_out], dim=1)
            x = rearrange(x, 'b n h w c -> b (n c) h w')

            state_params = [q1_bhwnc, k1_bhwnc, q2_bhwnc, k2_bhwnc, self.window_size, self.window_dilation, dilation]
            x = self.ssm(x, state_params=state_params)
        else:
            q_g = rearrange(q, 'b n h w c -> b n (h w) c')
            k_g = rearrange(k, 'b n h w c -> b n (h w) c')
            # global mode x
            x_g = rearrange(v, 'b n h w c -> b (n c) h w')
            v_g = rearrange(x_g, 'b (n c) h w -> b n (h w) c', n=self.num_heads, h=H, w=W)
            attn = einsum(q_g, k_g, 'b n l c, b n m c -> b n l m')
            
            if hasattr(self, 'rpb'):
                if self.rpb[0].shape[2:] != attn.shape[2:]:
                    rpb = F.interpolate(self.rpb[0], size=attn.shape[2:], mode='bicubic', align_corners=False)
                    attn = attn + rpb
                else:
                    attn = attn + self.rpb[0]
                
            attn = torch.softmax(attn, dim=-1)  
            x = einsum(attn, v_g, 'b n l m, b n m c -> b n c l').reshape(B, -1, H, W)
...
class FFN(nn.Module):
    def __init__(self, embed_dim, ffn_dim, act_layer=nn.GELU, dropout=0): 
        super().__init__()
...
class Block(nn.Module):
    def __init__(self, image_size=None, embed_dim=64, num_heads=2, window_size=7,
                 window_dilation=1, global_mode=False, use_rpb=False, sr_ratio=1,
                 ffn_dim=256, drop_path=0, layerscale=False, resscale=False,
                 layer_init_values=1e-6, token_mixer=Attention, channel_mixer=FFN, norm_layer=LayerNorm2d):
        super().__init__()
...
class BasicLayer(nn.Module):
    def __init__(self, image_size=None, embed_dim=64, depth=4, num_heads=4,
                 window_size=7, window_dilation=1, global_mode=False, use_rpb=False, sr_ratio=1,
                 ffn_dim=96, drop_path=0, layerscale=False, resscale=False,
        ...
        for i in range(depth):
            block = Block(embed_dim=embed_dim, num_heads=num_heads, window_size=window_size,
                          window_dilation=window_dilation, global_mode=global_mode, ffn_dim=ffn_dim,
                          drop_path=drop_path[i] if isinstance(drop_path, list) else drop_path,
                          layerscale=layerscale, resscale=resscale, layer_init_values=layer_init_values,
                          norm_layer=norm_layer, image_size=image_size, use_rpb=use_rpb, sr_ratio=sr_ratio)
...
class A2Mamba(nn.Module):
    def __init__(self,
        ...
        image_sizes = [(image_size_tuple[0]//2**(i+2), image_size_tuple[1]//2**(i+2)) for i in range(4)]
        
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                embed_dim=embed_dims[i_layer], depth=depths[i_layer], num_heads=num_heads[i_layer],
                window_size=window_size[i_layer], window_dilation=window_dilation[i_layer], global_mode=(i_layer==3),
                use_rpb=use_rpb, sr_ratio=sr_ratio[i_layer], ffn_dim=int(mlp_ratios[i_layer]*embed_dims[i_layer]),
                drop_path=dpr[sum(depths[:i_layer]):sum(depths[:i_layer + 1])], layerscale=layerscales[i_layer],
                resscale=resscales[i_layer], layer_init_values=layer_init_values[i_layer], norm_layer=norm_layer,
                image_size=image_sizes[i_layer], use_checkpoint=use_checkpoint[i_layer],
            )
        ...
        # 捨棄 dummy_input 推導，直接使用 embed_dims 以避免 YOLO 帶來的附帶效應並加速模型建立
        self.width_list = self.embed_dims
        try:
            self.eval() 
            dummy_input = torch.randn(1, in_chans, image_size_tuple[0], image_size_tuple[1])
            with torch.no_grad():
    ...
    def forward_features(self, x):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i%2 == 0:
                out.append(x)
        # 返回 list 確保 YOLO 不會拋出 'Tensor' object has no attribute 'insert'
        return out

    def forward(self, x):
        # 【修正重點】：為了配合 SMT 與 YOLO Backbone 的使用方式，直接返回多尺度特徵 List
...
# ---------------- 模型大小建立方法 ----------------
@register_model
def a2mamba_n(pretrained=False, image_size=224, **kwargs):
    model = A2Mamba(
...
@register_model
def a2mamba_t(pretrained=False, image_size=224, **kwargs):
    model = A2Mamba(
        image_size=image_size,
...
@register_model
def a2mamba_s(pretrained=False, image_size=224, **kwargs):
    model = A2Mamba(
        image_size=image_size,
...
@register_model
def a2mamba_b(pretrained=False, image_size=224, **kwargs):
    model = A2Mamba(
        image_size=image_size,
...
@register_model
def a2mamba_l(pretrained=False, image_size=224, **kwargs):
    model = A2Mamba(
        image_size=image_size,
