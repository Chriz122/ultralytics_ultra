...
class DetectionValidator(BaseValidator):
    ...
    def update_metrics(self, preds: list[dict[str, torch.Tensor]], batch: dict[str, Any]) -> None:
        ...
        for si, pred in enumerate(preds):
            ...
            # === 新增：計算單張圖片的 P 和 R (使用 IoU@0.5-0.95 平均) ===
            tp_array = batch_stats["tp"]  # shape: (N_preds, 10)
            
            # 修改處：將所有 10 個 IoU 閾值 (0.5~0.95) 的 True Positive 數量加總後除以閾值總數 (self.niou)
            tp_count = tp_array.sum() / self.niou if tp_array.size > 0 else 0 
            
            num_preds = len(predn["cls"])
            num_gts = len(pbatch["cls"])
            
            # 計算 Precision 與 Recall (若皆為 0，代表此圖本身無標註也無誤判，視為 Easy (1.0))
            img_p = tp_count / num_preds if num_preds > 0 else (1.0 if num_gts == 0 else 0.0)
            img_r = tp_count / num_gts if num_gts > 0 else (1.0 if num_preds == 0 else 0.0)
            
            # 將結果存入 dict，以圖片路徑為 Key
            self.image_metrics[pbatch["im_file"]] = (img_p, img_r)
            # ===================================================
            
            # # === 新增：計算單張圖片的 P 和 R (使用 IoU@0.5) ===
            # tp_array = batch_stats["tp"]  # shape: (N_preds, 10)
            # tp_count = tp_array[:, 0].sum() if tp_array.size > 0 else 0  # index 0 為 mAP@0.5
