        ...
        **kwargs
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                print("Warning: CUDA not available. Using fallback width_list.")

        except Exception as e:
            print(f"Warning during width_list calculation: {e}")
            self.to('cpu')
            self.train()
...
@register_model
def groupmamba_tiny(pretrained=False, img_size=224, **kwargs):
    model = GroupMamba(
        img_size=img_size,
        stem_hidden_dim = 32,
        embed_dims = [64, 128, 348, 448], 
...
@register_model
def groupmamba_small(pretrained=False, img_size=224, **kwargs):
    model = GroupMamba(
        img_size=img_size,
        stem_hidden_dim = 64,
...
@register_model
def groupmamba_base(pretrained=False, img_size=224, **kwargs):
    model = GroupMamba(
        img_size=img_size,
        stem_hidden_dim = 64,
...
if __name__ == '__main__':
    ...
    print("--- Creating GroupMamba Tiny model (CPU/CUDA compat test) ---")
    model = groupmamba_tiny(pretrained=False, img_size=img_h)
    
    if hasattr(model, 'width_list'):
        print("Calculated width_list:", model.width_list)
    
    # 1. Test CPU Forward (Simulate YOLO initialization stride check)
    print("\n1. Testing CPU Forward (Simulating YOLO init)...")
    input_cpu = torch.zeros(1, 3, img_h, img_w)
    try:
        model.eval()
        with torch.no_grad():
            out_cpu = model(input_cpu)
        print("CPU Forward Success! (Bypassed Mamba kernel)")
        print("Output shapes:", [o.shape for o in out_cpu])
    except Exception as e:
        print(f"CPU Forward Failed: {e}")
        import traceback
        traceback.print_exc()

    # 2. Test CUDA Forward (Simulate Training)
    if torch.cuda.is_available():
        print("\n2. Testing CUDA Forward (Real computation)...")
        device = torch.device("cuda")
        model.to(device)
        input_cuda = torch.rand(2, 3, img_h, img_w).to(device)
        try:
            out_cuda = model(input_cuda)
            print("CUDA Forward Success!")
            print("Output shapes:", [o.shape for o in out_cuda])
        except Exception as e:
             print(f"CUDA Forward Failed: {e}")
    else:
        print("\nSkipping CUDA test (No GPU detected)")