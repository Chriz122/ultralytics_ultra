...
class PosEmbMLPSwinv2D(nn.Module):
    ...
    def forward(self, input_tensor, local_window_size, cur_win_h=None, cur_win_w=None):
        ...
        if not self.no_log:
            ...
            # 移除了 numpy 依賴，將 np.log2(8) 替換為常數 3.0
...
# -------------------------------------------------------------
# Attention Residuals (AttnRes) 模組
# 依據論文概念，將先前的增量資訊以 Softmax 權重做適配聚合
# 支援 AMP 穩定運算 (Float32 cast)
# -------------------------------------------------------------
class StageAttnRes(nn.Module):
    def __init__(self, dim, is_2d=False):
        super().__init__()
        # 依論文要求，pseudo-query vectors 初始化為零，以在訓練初期等同平均權重
        self.proj = nn.Parameter(torch.zeros(dim))
        self.is_2d = is_2d
        self.norm_weight = nn.Parameter(torch.ones(dim))
        self.eps = 1e-6
        
    def forward(self, blocks):
        # blocks: 一個 list，包含殘差流的增量歷史
        V = torch.stack(blocks, dim=0) 
        
        # 轉換為 float32 確保在 AMP 訓練下的數值穩定性
        V_f = V.float()
        
        if self.is_2d:
            # 針對 CNN (B, C, H, W)
            norm_x = V_f.pow(2).mean(dim=2, keepdim=True)
            K_norm = V_f * torch.rsqrt(norm_x + self.eps)
            K_norm = K_norm * self.norm_weight.view(1, 1, -1, 1, 1).float()
            
            logits = torch.einsum('c, n b c h w -> n b h w', self.proj.float(), K_norm)
            attn = logits.softmax(dim=0).to(V.dtype)
            h = torch.einsum('n b h w, n b c h w -> b c h w', attn, V)
        else:
            # 針對 Transformer (B, T, C)
            norm_x = V_f.pow(2).mean(dim=-1, keepdim=True)
            K_norm = V_f * torch.rsqrt(norm_x + self.eps)
            K_norm = K_norm * self.norm_weight.view(1, 1, 1, -1).float()
            
            logits = torch.einsum('c, n b t c -> n b t', self.proj.float(), K_norm)
            attn = logits.softmax(dim=0).to(V.dtype)
            h = torch.einsum('n b t, n b t c -> b t c', attn, V)
...
class FasterViTLayer(nn.Module):
    def __init__(self,
        ...
        # 實例化 Attention Residuals
        self.attn_res_x = nn.ModuleList([StageAttnRes(dim, is_2d=self.conv) for _ in range(depth)])
        self.final_attn_res_x = StageAttnRes(dim, is_2d=self.conv)
        
        if self.do_gt:
            self.attn_res_ct = nn.ModuleList([StageAttnRes(dim, is_2d=False) for _ in range(depth)])
            self.final_attn_res_ct = StageAttnRes(dim, is_2d=False)
        else:
            self.attn_res_ct = None
            self.final_attn_res_ct = None
    ...
    def forward(self, x):
        ...
        # 紀錄先前的 block representations 以進行 AttnRes 聚合
        blocks_x = [x]
        blocks_ct = [ct] if ct is not None else None
            
        for bn, blk in enumerate(self.blocks):
            # 取代原本以線性相加的邏輯，將歷來增量聚合送給下一層做運算
            h_x = self.attn_res_x[bn](blocks_x)
            h_ct = self.attn_res_ct[bn](blocks_ct) if ct is not None else None
            
            if isinstance(blk, HAT):
                out_x, out_ct = blk(h_x, h_ct, num_windows_h, num_windows_w)
            else:
                out_x, out_ct = blk(h_x, h_ct)
                
            # 抽取出當前 Block 造成的增量(increment)，推入歷史中以供 Attention
            inc_x = out_x - h_x
            blocks_x.append(inc_x)
            
            if ct is not None:
                inc_ct = out_ct - h_ct
                blocks_ct.append(inc_ct)
                
        # Block AttnRes 最終匯總
        x = self.final_attn_res_x(blocks_x)
        if ct is not None:
            ct = self.final_attn_res_ct(blocks_ct)
...
@register_model
def faster_vit_0_AttnRes(pretrained=False, resolution=224, **kwargs):
...
@register_model
def faster_vit_1_AttnRes(pretrained=False, resolution=224, **kwargs):
...
@register_model
def faster_vit_2_AttnRes(pretrained=False, resolution=224, **kwargs):
...
@register_model
def faster_vit_3_AttnRes(pretrained=False, resolution=224, **kwargs):
...
@register_model
def faster_vit_4_AttnRes(pretrained=False, resolution=224, **kwargs):
    model = FasterViT(depths=[3, 3, 12, 5],
                      num_heads=[4, 8, 16, 32],
                      window_size=[7, 7, 7, 7],
                      ct_size=2,
...
@register_model
def faster_vit_5_AttnRes(pretrained=False, resolution=224, **kwargs):
    model = FasterViT(depths=[3, 3, 12, 5],
                      num_heads=[4, 8, 16, 32],
                      window_size=[7, 7, 7, 7],
                      ct_size=2,
...
@register_model
def faster_vit_6_AttnRes(pretrained=False, resolution=224, **kwargs):
