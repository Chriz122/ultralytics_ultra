...
class PoseTrainer(yolo.detect.DetectionTrainer):
    ...
    def get_validator(self):
        ...
        # --- 修改部分開始 ---
        # 檢查模型配置中是否開啟了 rtmo
        is_rtmo = getattr(self.model, "yaml", {}).get("rtmo", False)
        is_rlepose = getattr(self.model, "yaml", {}).get("rlepose", False)

        if is_rtmo:
            # 當 rtmo 為 True 時，加入 mle_loss
            self.loss_names = "box_loss", "proxy_loss", "kobj_loss", "cls_loss", "dfl_loss", "mle_loss", "moe_loss"
        # elif is_rlepose:
        #     # 當 rlepose 為 True 時，加入 rlepose_loss
        #     self.loss_names = "box_loss", "pose_loss", "kobj_loss", "cls_loss", "dfl_loss", "rle_loss", "moe_loss"
        else:
            # 原始的損失組件
            self.loss_names = "box_loss", "pose_loss", "kobj_loss", "cls_loss", "dfl_loss", "moe_loss"
        
        # 額外檢查是否存在 flow_model (例如 RLE)
