...
# Added: Block Attention Residuals from "ATTENTION RESIDUALS" paper
...
#########################################    ATTENTION RESIDUALS MODULES     ########################################

class RMSNorm2d(nn.Module):
    """2D RMSNorm adapted for keys in Block Attention Residuals"""
    ...
    def forward(self, x):
        # x shape can be [B, C, H, W] or [N, B, C, H, W]
        # normalize along the channel dimension (index -3)
        norm = torch.mean(x ** 2, dim=-3, keepdim=True)
        
        # Reshape weight to broadcast over spatial dims
        shape = [1] * x.dim()
        shape[-3] = self.weight.shape[0]
        weight = self.weight.view(shape)
        
        return x * torch.rsqrt(norm + self.eps) * weight

class BlockAttnResContainer(nn.Module):
    """
    Implements Block Attention Residuals from the paper.
    Groups a list of sequential blocks and applies dynamic depth-wise attention.
    """
    def __init__(self, layers: list, channels: int, block_size: int = 2):
        super().__init__()
        self.layers = nn.ModuleList(layers)
        self.block_size = block_size
        
        # Learnable pseudo-queries (w_l), initialized to zero as per paper
        self.queries = nn.ParameterList([
            nn.Parameter(torch.zeros(1, channels, 1, 1)) for _ in layers
        ])
        
        # RMSNorm applied exclusively on the Keys
        self.norms = nn.ModuleList([
            RMSNorm2d(channels) for _ in layers
        ])

    def forward(self, x):
        # b_0 is the initial token embedding / stage input feature
        blocks = [x]
        # b_n^0 is initialized to 0 (the current block's partial sum)
        partial_block = torch.zeros_like(x)
        
        for i, layer in enumerate(self.layers):
            block_rel_idx = i % self.block_size
            
            # 1. Gather inter-block and intra-block sources
            if block_rel_idx == 0:
                V_list = blocks # First layer of a block only attends to completed past blocks
            else:
                V_list = blocks + [partial_block] # Subsequent layers attend to past + partial sum
                
            V = torch.stack(V_list, dim=0) # [Num_sources, B, C, H, W]
            
            # 2. Key Projection & Logits Calculation
            K = self.norms[i](V)
            w = self.queries[i].unsqueeze(0) # [1, 1, C, 1, 1]
            
            # depth-wise linear attention (dot product across channels)
            logits = torch.sum(w * K, dim=2) # [Num_sources, B, H, W]
            
            # 3. Softmax Attention Weights
            attn_weights = torch.softmax(logits, dim=0).unsqueeze(2) # [Num_sources, B, 1, H, W]
            
            # 4. Aggregation
            h = torch.sum(attn_weights * V, dim=0) # [B, C, H, W]
            
            # 5. Layer Execution
            # In CPUBone, standard residual is already inside the block: out = h + f_l(h)
            out = layer(h)
            
            # 6. Extract Delta (f_l(h_l)) & Update Partial Sum
            delta = out - h
            partial_block = partial_block + delta
            
            # 7. Block Boundaries Management
            if block_rel_idx == self.block_size - 1 and i != len(self.layers) - 1:
                blocks.append(partial_block)
                partial_block = torch.zeros_like(x)
                
        # Return final aggregation combining all full blocks and the last partial block
        return sum(blocks) + partial_block
...
class CPUBoneBackbone(nn.Module):
    def __init__(self, width_list: list[int], depth_list: list[int], in_channels=3, img_size=224, expand_ratio=4, norm="bn2d", act_func="hswish", bb_convattention=False, bb_convin2=False, fastit=False, huge_model=False, bigit=False, grouping=1, smallk_only_lasts=False, lose_transpose=False, just_unfused=False, use_attn_res=True, attn_res_block_size=2) -> None:
        super().__init__()
        ...
        #         else:
        #             sub_blocks.append(ResidualBlock(block, IdentityLayer()))
        #         curr_channels = w
                
        #     # Insert Block Attention Residual Container if enabled
        #     if self.use_attn_res and len(sub_blocks) > 0:
        #         stage.append(BlockAttnResContainer(sub_blocks, channels=curr_channels, block_size=self.attn_res_block_size))
        #     else:
        #         stage.extend(sub_blocks)
...
if __name__ == '__main__':
    ...
    # 測試含有 Attention Residuals 的 B1 模型
    print("--- Creating CPUBone B1 model with Block Attention Residuals ---")
    model = cpubone_b1(use_attn_res=True, attn_res_block_size=2)
