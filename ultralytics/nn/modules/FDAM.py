# --------------------------------------------------------
# Linwei Chen (Modified for YOLO11 compatibility & no MMCV/MMSeg/Numpy)
# --------------------------------------------------------
import math
import logging
import torch
...
class GroupDynamicScale(nn.Module):
    def __init__(self, dim, expansion_ratio=1, reweight_expansion_ratio=.125,
        ...
        # 安全機制：確保 group 能整除 dim 以避免維度不匹配的報錯
        self.group = math.gcd(dim, group)
        self.med_channels = int(expansion_ratio * dim)
        self.weight_resize = weight_resize
        
        self.reweight = Mlp(dim, reweight_expansion_ratio, self.group * num_filters, bias=False)
        self.complex_weights = nn.Parameter(
            torch.randn(num_filters, dim // self.group, self.size, self.filter_size, dtype=torch.float32) * init_scale)
...
# =====================================================================
# FDAM Blocks (Adapted to perfectly substitute Ultralytics C2PSA block)
# =====================================================================
class FdamLayer(nn.Module):
    """
    Core FDAM calculation operating inherently on (B, C, H, W)
    """
    def __init__(self, dim, num_heads, mlp_ratio=4., drop_path=0., norm_layer=nn.LayerNorm, init_values=1e-4):
        ...
        # 【修正這裡】: 改回使用 dim 與 mlp_ratio 來符合自定義 Mlp 類的簽名
    ...
    def forward(self, x):
        # 接收 YOLO 架構預設的 (B, C, H, W) 張量
        B, C, H, W = x.shape
        x_nlc = x.flatten(2).transpose(1, 2)
        
        # --- Attention block with FDAM ---
        x_att = self.attn(self.norm1(x_nlc))
        ...
        # --- MLP block with FDAM ---
        x_mlp = self.mlp(self.norm2(x_nlc))
        ...
        # 自動轉回 (B, C, H, W) 以符合 YOLO 下一模塊接口
        return x_nlc.transpose(1, 2).reshape(B, C, H, W)


class FdamBlock(nn.Module):
    """
    Drop-in replacement for `C2PSA` compatible directly with YOLO11 yaml files.
    Parameters match Ultralytics expected sequence: (c1, c2, n, e).
    """
    def __init__(self, c1, c2, n=1, e=0.5):
        super().__init__()
        assert c1 == c2, "c1 and c2 must be equal in FdamBlock/C2PSA"
        self.c = int(c1 * e)
        # 用 1x1 Convolution 切出兩個分支
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1, 1)
        
        # FDAM 串列
        self.m = nn.Sequential(*(FdamLayer(dim=self.c, num_heads=max(1, self.c // 64)) for _ in range(n)))

    def forward(self, x):
        # 進行 Cross Stage Partial 切分處理
        a, b = self.cv1(x).split((self.c, self.c), dim=1)
        b = self.m(b)
        return self.cv2(torch.cat((a, b), 1))


class vit_models_freq(nn.Module):
