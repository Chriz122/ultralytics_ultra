...
from ultralytics.nn.modules import (
    ...
    SwinTransformer_RoPE_Tiny, 
    SwinTransformer_RoPE_Tiny_c24, 
    SwinTransformer_RoPE_Small, 
    SwinTransformer_RoPE_Base, 
    SwinTransformer_RoPE_Large,
    SwinTransformer_ViT5_Tiny, 
    SwinTransformer_ViT5_Small, 
    SwinTransformer_ViT5_Base, 
    SwinTransformer_ViT5_Large,
    EMO_1M, 
    EMO_2M, 
    EMO_5M, 
    EMO_6M,
...
class PoseModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        # 1. 檢查 YAML 配置中是否啟用了 RTMO
        # 通常 cfg 內容會被儲存在 self.yaml 中
        is_rtmo = getattr(self, "yaml", {}).get("rtmo", False)
        is_rlepose = getattr(self, "yaml", {}).get("rlepose", False)

        if is_rtmo:
            return RTMOPoseLoss(self)

        if is_rlepose:
            return RLEPoseLoss(self)
        
        # 2. 原有的 End-to-End 或標準 Pose Loss 邏輯
...
def parse_model(d, ch, verbose=True):
    ...
    nc, act, scales, end2end, rtmo, rlepose = (d.get(x) for x in ("nc", "activation", "scales", "end2end", "rtmo", "rlepose"))
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {C3k2, C3k2_ConvNeXtV2Block, C3k2_FasterBlock, C3k2_WTConv, C3k2_SAConv, C3k2_RepVGG, C3k2_OREPA_backbone, C3k2_OREPA_neck,
                     C3k2_MLLABlock1, C3k2_MLLABlock2, C3k2_DiTBlock, C3k2_UIB, C3k2_DSConv, DSC3k2, C3k2PBA, C3k2PCA, C3k2_AttnRes}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
            ...
            # 總輸出通道數等於兩者相加
            c2 = sum(c1)
            # 強制將 c1, c2 插入到 args 中，傳給 FreqFusion 的 __init__
            args = [c1, c2, *args]
        # =========== 新增這一段 ===========
        elif m is LateralConnection:
            c2 = sum(ch[x] for x in f)
            # 將上一層與橫向連接的通道數列表，作為第一個參數傳遞給模組
        ...
        # ==================================
            ...
            args = [*args[1:]]
                   ...
                   MobileMamba_B1, MobileMamba_B2, MobileMamba_B4, vcmamba_efficientformer_s, vcmamba_efficientformer_m, vcmamba_efficientformer_b, defm_tiny, defm_small, defm_base, waveformer_tiny, 
                   waveformer_small, waveformer_base, faster_vit_0, faster_vit_1, faster_vit_2, faster_vit_3, faster_vit_4, faster_vit_5, faster_vit_6, SwinTransformer_RoPE_Tiny, SwinTransformer_RoPE_Tiny_c24, 
