...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        # (b, max_num_obj, 1, 4), (b, 1, h*w, 4)  [OBB 則為 5]
        pd_boxes = pd_bboxes.unsqueeze(1).expand(-1, self.n_max_boxes, -1, -1)[mask_gt]
        gt_boxes = gt_bboxes.unsqueeze(2).expand(-1, -1, na, -1)[mask_gt]
        
        # ★ 恢復動態調用，這樣 OBB 就會自動去調用 RotatedTaskAlignedAssigner 的 probiou
    ...
    def iou_calculation(self, gt_bboxes, pd_bboxes):
        ...
        # ★ MPDIoU 在 TaskAlignedAssigner 中因為缺少 hw 參數，故統一用 CIoU 代替
        if iou_type == 'mpdiou':
            iou_type = 'CIoU'
        ...
        return bbox_iou(gt_bboxes, pd_bboxes, xywh=False, 
                        ...
                        mpdiou=mpdiou, Inner=Inner, Focaleriou=Focaleriou,
                        d=d, u=u, ratio=ratio, eps=1e-7, Lambda=Lambda, scale=scale).squeeze(-1).clamp_(0)
