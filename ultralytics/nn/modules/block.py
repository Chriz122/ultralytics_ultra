...
class StripBottleneck(nn.Module):
    def __init__(self, d_model, k1, k2, shortcut=True):
        super().__init__()
        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = StripModule(d_model, k1, k2)
        self.proj_2 = nn.Conv2d(d_model, d_model, 1)
        
        # 加入 shortcut 判斷，對齊原本 Bottleneck 的設計
        self.add = shortcut 
    ...
    def forward(self, x):
        ...
        # 根據 shortcut 參數決定是否進行殘差連接
        return x + shortcut if self.add else x


class C2f_Strip(nn.Module):
    """Faster Implementation of CSP Bottleneck with StripBlock."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5, k1: int = 1, k2: int = 19):
        """Initialize a CSP bottleneck with StripBlock.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            n (int): Number of Strip blocks.
            shortcut (bool): Whether to use shortcut connections.
            g (int): Groups for convolutions (kept for compatibility, unused in StripBlock).
            e (float): Expansion ratio.
            k1 (int): Kernel size 1 for StripModule.
            k2 (int): Kernel size 2 for StripModule.
        """
        super().__init__()
        self.c = int(c2 * e)  # hidden channels
        
        # 注意：這裡的 Conv 需要替換為您環境中實際定義的 Conv (如 Ultralytics 的 Conv)
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        
        # ★ 核心修改：將原本的 Bottleneck 替換為 StripBottleneck
        self.m = nn.ModuleList(
            StripBottleneck(d_model=self.c, k1=k1, k2=k2, shortcut=shortcut) for _ in range(n)
        )
...
class RTMBlock(nn.Module):
    """CSP Bottleneck with 3 convolutions (Aligned with image CSPLayer)."""
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.attention = ChannelAttention(2 * c_)  
        
        # 【修改點 1】：將 k 設為 (3, 5) 以符合圖片中的 Conv(k=3) 與 DWConv(k=5)
        # 保持 e=1.0 確保內部展開的通道數 c=c_out
        self.m = nn.Sequential(*(CSPNeXtBlock(c_, c_, shortcut, g, k=(3, 5), e=1.0) for _ in range(n)))

    def forward(self, x):
        """Forward pass through the CSP bottleneck."""
        # 先將兩條分支的結果 Concat 起來
        x_concat = torch.cat((self.m(self.cv1(x)), self.cv2(x)), 1)
        
        # 【修改點 2】：依照圖片順序，Concat 後先經過 ChannelAttention，最後再經過 cv3 (ConvModule)
        return self.cv3(self.attention(x_concat))
...
# ---------------------------------------------------------
# 新增: LaSt-ViT (LazyStrike) 核心模塊，針對密集特徵圖適配
# ---------------------------------------------------------
class LazyStrike(nn.Module):
    """
    LazyStrike module adapted from "Vision Transformers Need More Than Registers".
    Calculates stability scores using channel-wise 1D FFT and masks out lazy background tokens.
    """
    def __init__(self, dim, keep_ratio=0.5):
        super().__init__()
        self.keep_ratio = keep_ratio
        
        # Pre-compute and register the 1D Gaussian kernel
        kernel = self.gaussian_kernel_1d(dim, dim ** 0.5)
        self.register_buffer("gs_k", kernel.view(1, 1, dim))

    def gaussian_kernel_1d(self, kernel_size, sigma):
        x = torch.arange(-kernel_size // 2 + 1, kernel_size // 2 + 1).float()
    ...
    def forward(self, v):
        """
        v: Tensor of shape (B, N, C)
        """
        B, N, C = v.shape
        
        # FFT often requires float32 to avoid precision/support issues in half-precision training
        v_float = v.float()
        
        # 1. 1D FFT and Low-pass filtering
        v_freq = torch.fft.fft(v_float, dim=-1)
        v_freq = torch.fft.fftshift(v_freq, dim=-1)
        v_freq = v_freq * self.gs_k
        v_freq = torch.fft.ifftshift(v_freq, dim=-1)
        v_lp = torch.fft.ifft(v_freq, dim=-1).real
        
        # 2. Compute Stability Score (diff)
        eps = 1e-6
        diff = v_float / (torch.abs(v_lp - v_float) + eps)
        
        # 3. Channel-wise Top-K Selection
        ...
        # Apply mask to original precision tensor
        return v * mask.to(v.dtype)


class LaSt_AAttn(nn.Module):
    """
    Area-attention module integrated with LazyStrike to prevent lazy aggregation.
    """
    def __init__(self, dim, num_heads, area=1, use_last=True, last_ratio=0.5):
        super().__init__()
        self.area = area
        self.num_heads = num_heads
        self.head_dim = head_dim = dim // num_heads
        ...
        # LazyStrike initialization
        self.use_last = use_last
        if self.use_last:
            self.lazy_strike = LazyStrike(all_head_dim, keep_ratio=last_ratio)
    ...
    def forward(self, x):
        ...
        # Extract Value and process with LazyStrike before attention
        v = self.v(x)
        pp = self.pe(v)
        v = v.flatten(2).transpose(1, 2)  # Shape: (B, N, C_all)

        # 核心改動：對 Value 矩陣應用 LazyStrike 遮罩，消除背景捷徑
        if self.use_last:
            v = self.lazy_strike(v)
...
class LaSt_ABlock(nn.Module):
    """
    ABlock class implementing a Area-Attention block with effective feature extraction and LazyStrike.
    """
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1, use_last=True, last_ratio=0.5):
        super().__init__()

        self.attn = LaSt_AAttn(dim, num_heads=num_heads, area=area, use_last=use_last, last_ratio=last_ratio)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = nn.Sequential(Conv(dim, mlp_hidden_dim, 1), Conv(mlp_hidden_dim, dim, 1, act=False))
...
class LaSt_A2C2f(nn.Module):  
    """
    A2C2f module with residual enhanced feature extraction and LaSt-ViT improvements.
    """
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True, use_last=True, last_ratio=0.5):
        super().__init__()
        c_ = int(c2 * e)
        assert c_ % 32 == 0, "Dimension of ABlock be a multiple of 32."
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(LaSt_ABlock(c_, num_heads, mlp_ratio, area, use_last, last_ratio) for _ in range(2))) if a2 else C3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
...
# ---------------------------------------------------------
# 新增: ViT-5 (RMSNorm、RoPE) 核心模塊，針對密集特徵圖適配
# ---------------------------------------------------------
def rotate_half(x):
...
class RMSNorm(nn.Module):
    """RMSNorm applied to sequence components."""
...
class RMSNorm_spatial(nn.Module):
    """RMSNorm applied to spatial 2D feature maps (B, C, H, W)."""
    def __init__(self, hidden_size, eps=1e-6):
        super().__init__()
...
class VisionRotaryEmbedding(nn.Module):
    """2D RoPE adapting to spatial maps and avoiding Numpy."""
    def __init__(self, dim, pt_seq_len=14, theta=10000):
        super().__init__()
        freqs = 1. / (theta ** (torch.arange(0, dim, 2)[:(dim // 2)].float() / dim))
        self.pt_seq_len = pt_seq_len
        self.register_buffer("freqs", freqs)

    def forward(self, H, W, B, area, device, dtype=torch.float32): 
        ty = torch.arange(H, device=device) / max(H, 1) * self.pt_seq_len
        tx = torch.arange(W, device=device) / max(W, 1) * self.pt_seq_len

        freqs_y = torch.einsum('..., f -> ... f', ty, self.freqs)
        freqs_y = repeat(freqs_y, '... n -> ... (n r)', r=2) 
        
        freqs_x = torch.einsum('..., f -> ... f', tx, self.freqs)
        ...
        N = H * W
        freqs = freqs.view(1, N, 1, freqs.shape[-1]) 
        
        if area > 1:
            freqs = freqs.view(1, area, N // area, 1, freqs.shape[-1])
            freqs = freqs.expand(B, area, N // area, 1, freqs.shape[-1]).reshape(B * area, N // area, 1, freqs.shape[-1])
        else:
            freqs = freqs.expand(B, N, 1, freqs.shape[-1])
            
        # 修復：將算好的 RoPE 的 sin、cos 手動轉回輸入對象對應的 dtype (例如 FP16)
        return freqs.cos().to(dtype), freqs.sin().to(dtype)


class AAttn_5(nn.Module):
    """
    ViT-5 Modernized Area-attention module.
    Features QK-Norm, 2D RoPE, Bias-Free Projections, and Learnable Registers.
    """
    def __init__(self, dim, num_heads, area=1, num_registers=4, qk_norm=True):
        super().__init__()
        self.area = area
        self.num_heads = num_heads
        self.head_dim = head_dim = dim // num_heads
        ...
        # ViT-5: We remove bias terms in the QKV projection layers
        ...
        self.pe = nn.Conv2d(all_head_dim, dim, 5, 1, 2, groups=dim, bias=False)
        ...
        # ViT-5: 2D RoPE
        self.rope = VisionRotaryEmbedding(head_dim // 2, theta=10000)

        # ViT-5: Register Tokens 
        self.num_registers = num_registers
        if num_registers > 0:
            self.rope_reg_size = int(math.isqrt(num_registers))
            assert self.rope_reg_size ** 2 == num_registers, "num_registers must be a square number"
            self.q_reg = nn.Parameter(torch.zeros(1, num_registers, num_heads, head_dim))
            self.k_reg = nn.Parameter(torch.zeros(1, num_registers, num_heads, head_dim))
            self.v_reg = nn.Parameter(torch.zeros(1, num_registers, num_heads, head_dim))
            nn.init.trunc_normal_(self.q_reg, std=.02)
            nn.init.trunc_normal_(self.k_reg, std=.02)
            nn.init.trunc_normal_(self.v_reg, std=.02)
            
            # Registers need high-frequency separate 2D RoPE bases (theta=100)
            self.rope_reg = VisionRotaryEmbedding(head_dim // 2, pt_seq_len=self.rope_reg_size, theta=100)
    ...
    def forward(self, x):
        ...
        v_orig = self.v(x)
        pp = self.pe(v_orig)
        v = v_orig.flatten(2).transpose(1, 2)

        if self.area > 1:
            qk = qk.reshape(B * self.area, N // self.area, C * 2)
            v = v.reshape(B * self.area, N // self.area, C)
            B_area, N_area = B * self.area, N // self.area
        else:
            B_area, N_area = B, N

        q, k = qk.split([C, C], dim=2)

        q = q.view(B_area, N_area, self.num_heads, self.head_dim)
        k = k.view(B_area, N_area, self.num_heads, self.head_dim)
        v = v.view(B_area, N_area, self.num_heads, self.head_dim)
        ...
        # Apply spatial 2D RoPE
        # 修復：傳遞 x.dtype 保證矩陣乘法不會溢出至 FP32
        freqs_cos, freqs_sin = self.rope(H, W, B, self.area, x.device, x.dtype)
        q = q * freqs_cos + rotate_half(q) * freqs_sin
        k = k * freqs_cos + rotate_half(k) * freqs_sin

        # Dynamic Appending of Register Tokens
        if self.num_registers > 0:
            q_reg = self.q_reg.expand(B_area, self.num_registers, -1, -1)
            k_reg = self.k_reg.expand(B_area, self.num_registers, -1, -1)
            v_reg = self.v_reg.expand(B_area, self.num_registers, -1, -1)
            
            if self.qk_norm:
                q_reg = self.q_norm(q_reg).to(x.dtype)
                k_reg = self.k_norm(k_reg).to(x.dtype)
                
            freqs_cos_reg, freqs_sin_reg = self.rope_reg(self.rope_reg_size, self.rope_reg_size, B_area, 1, x.device, x.dtype)
            q_reg = q_reg * freqs_cos_reg + rotate_half(q_reg) * freqs_sin_reg
            k_reg = k_reg * freqs_cos_reg + rotate_half(k_reg) * freqs_sin_reg
                
            q = torch.cat([q, q_reg], dim=1)
            k = torch.cat([k, k_reg], dim=1)
            v = torch.cat([v, v_reg], dim=1)

        # Execution of Flash Attention / Standard Attention
        if x.is_cuda and USE_FLASH_ATTN:
            # Flash Attention 只接受 fp16/bf16，需要安全轉型
            attn_dtype = q.dtype if q.dtype in (torch.float16, torch.bfloat16) else torch.float16
            x_attn = flash_attn_func(
                q.contiguous().to(attn_dtype),
                k.contiguous().to(attn_dtype),
                v.contiguous().to(attn_dtype)
            ).to(x.dtype)
            
            x_attn = x_attn[:, :N_area, :, :] # Remove registers
            x_attn = x_attn.reshape(B_area, N_area, C)
        else:
            q = q.transpose(1, 2)
            k = k.transpose(1, 2)
            v = v.transpose(1, 2)
            ...
            x_attn = x_attn.transpose(1, 2)[:, :N_area, :, :] # Remove registers
            x_attn = x_attn.reshape(B_area, N_area, C)

        # Restore original spatial shape mapping
        if self.area > 1:
...
class ABlock_5(nn.Module):
    """
    ViT-5 Modernized ABlock.
    Incorporating Pre-RMSNorm, LayerScale and preventing SwiGLU over-gating.
    """
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1, num_registers=4, qk_norm=True, layer_scale=True, init_values=1e-4):
        super().__init__()
        
        self.norm1 = RMSNorm_spatial(dim, eps=1e-6)
        self.attn = AAttn_5(dim, num_heads=num_heads, area=area, num_registers=num_registers, qk_norm=qk_norm)
        
        self.norm2 = RMSNorm_spatial(dim, eps=1e-6)
        mlp_hidden_dim = int(dim * mlp_ratio)
        
        # ViT-5: To avoid sparsity logic issue of combining SwiGLU and LayerScale, uses explicit GELU formulation.
    ...
    def forward(self, x):
        if self.layer_scale:
            # 加入 to(x.dtype) 安全防護避免被 FloatTensor 污染
            x = x + (self.gamma_1 * self.attn(self.norm1(x))).to(x.dtype)
            x = x + (self.gamma_2 * self.mlp(self.norm2(x))).to(x.dtype)
        else:
...
class A2C2f_5(nn.Module):  
    """
    A2C2f module utilizing modernized ViT-5 ABlock mechanics with R-ELAN mapping.
    """
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        ...
        # 這裡假設您的環境中已經導入了 Conv / C3k (例如：from ultralytics.nn.modules import Conv, C3k)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv((1 + n) * c_, c2, 1)
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock_5(c_, num_heads, mlp_ratio, area) for _ in range(2))) if a2 else C3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
    ...
    def forward(self, x):
        ...
        y.extend(m(y[-1]) for m in self.m)
        if self.gamma is not None:
            # 同樣為殘差合併添加 dtype 強制轉換
            return x + (self.gamma.view(1, -1, 1, 1) * self.cv2(torch.cat(y, 1))).to(x.dtype)
        return self.cv2(torch.cat(y, 1))

# ---------------------------------------------------------
# 新增: RoPEMixed 核心模塊，針對密集特徵圖適配
# ---------------------------------------------------------
class VisionRotaryEmbeddingMixed(nn.Module):
    """2D RoPE-Mixed: Adapting to spatial maps with learnable mixed-axis frequencies."""
    def __init__(self, head_dim, num_heads, theta=10.0, rotate=True):
        super().__init__()
        assert head_dim % 4 == 0, "head_dim must be divisible by 4 for RoPE-Mixed"
        ...
        # [num_heads, head_dim // 2] - RoPE-Mixed 的核心: 讓頻率矩陣可學習 (Learnable Parameter)
        self.freqs_x = nn.Parameter(torch.stack(freqs_x, dim=0), requires_grad=True)
        self.freqs_y = nn.Parameter(torch.stack(freqs_y, dim=0), requires_grad=True)

    def forward(self, H, W, B, area, device, dtype=torch.float32):
        # 關閉 AMP 以避免生成網格與指數相加時發生 Float16 溢出
        with torch.cuda.amp.autocast(enabled=False):
            t_x = torch.arange(W, device=device, dtype=torch.float32)
            t_y = torch.arange(H, device=device, dtype=torch.float32)
            
            # 【修復點】：將 'hd' 改為 'nd' 以避免和前面的 'h' (高度) 發生字母衝突
            fx = torch.einsum('w, nd -> w n d', t_x, self.freqs_x.float())
            fy = torch.einsum('h, nd -> h n d', t_y, self.freqs_y.float())
            
            #[H, W, num_heads, head_dim // 2] 混合雙軸
            freqs = fx.unsqueeze(0) + fy.unsqueeze(1) 
            
            N = H * W
            freqs = freqs.view(N, self.num_heads, self.head_dim // 2)
            
            # 配對 Expand 用於 rotate_half (將 [f1, f2] 重複為[f1, f1, f2, f2])
            freqs = freqs.unsqueeze(-1).expand(*freqs.shape, 2).reshape(N, self.num_heads, self.head_dim)
            
            if area > 1:
                freqs = freqs.view(1, area, N // area, self.num_heads, self.head_dim)
                freqs = freqs.expand(B, area, N // area, self.num_heads, self.head_dim)
                freqs = freqs.reshape(B * area, N // area, self.num_heads, self.head_dim)
            else:
                freqs = freqs.view(1, N, self.num_heads, self.head_dim)
                freqs = freqs.expand(B, N, self.num_heads, self.head_dim)
                
            freqs_cos = freqs.cos().to(dtype)
            freqs_sin = freqs.sin().to(dtype)
            
        return freqs_cos, freqs_sin


class AAttn_5_RoPEMixed(nn.Module):
    """
    ViT-5 Modernized Area-attention module.
    Features QK-Norm, RoPE-Mixed, Bias-Free Projections, and Learnable Registers.
    """
    def __init__(self, dim, num_heads, area=1, num_registers=4, qk_norm=True):
        super().__init__()
        self.area = area
        ...
        # ViT-5: 2D RoPE-Mixed
        self.rope = VisionRotaryEmbeddingMixed(head_dim, num_heads, theta=10.0, rotate=True)
        ...
        if num_registers > 0:
            ...
            # Registers need high-frequency separate RoPE-Mixed bases
            self.rope_reg = VisionRotaryEmbeddingMixed(head_dim, num_heads, theta=100.0, rotate=True)
    ...
    def forward(self, x):
        ...
        # Flash Attention / Standard Attention (支援 AMP fp16/bf16 格式)
        if x.is_cuda and USE_FLASH_ATTN:
            attn_dtype = q.dtype if q.dtype in (torch.float16, torch.bfloat16) else torch.float16
            x_attn = flash_attn_func(
                q.contiguous().to(attn_dtype),
            ...
            x_attn = x_attn.transpose(1, 2)[:, :N_area, :, :] 
            x_attn = x_attn.reshape(B_area, N_area, C)
...
class ABlock_5_RoPEMixed(nn.Module):
    """
    ViT-5 Modernized ABlock.
    Incorporating Pre-RMSNorm, LayerScale, RoPE-Mixed, timm DropPath and preventing SwiGLU over-gating.
    """
    def __init__(self, dim, num_heads, mlp_ratio=1.2, area=1, num_registers=4, qk_norm=True, layer_scale=True, init_values=1e-4, drop_path=0.):
        super().__init__()
        
        self.norm1 = RMSNorm_spatial(dim, eps=1e-6)
        self.attn = AAttn_5_RoPEMixed(dim, num_heads=num_heads, area=area, num_registers=num_registers, qk_norm=qk_norm)
    ...
    def forward(self, x):
        if self.layer_scale:
            x = x + self.drop_path((self.gamma_1 * self.attn(self.norm1(x))).to(x.dtype))
            x = x + self.drop_path((self.gamma_2 * self.mlp(self.norm2(x))).to(x.dtype))
        else:
...
class A2C2f_5_RoPEMixed(nn.Module):  
    """
    A2C2f module utilizing modernized ViT-5 ABlock mechanics with R-ELAN mapping.
    """
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True, drop_path=0.):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        assert c_ % 32 == 0, "Dimension of ABlock be a multiple of 32."
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock_5_RoPEMixed(c_, num_heads, mlp_ratio, area, drop_path=drop_path) for _ in range(2))) if a2 else C3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
...
# ---------------------------------------------------------
# 新增: 基於 Attention Residuals 論文設計的空間特徵聚合模塊
# ---------------------------------------------------------
class SpatialAttnRes(nn.Module):
    ...
    def __init__(self, dim):
        ...
        # 論文規定: 使用 RMSNorm 來限制 Key 的幅度，避免具有較大輸出的層主導 Softmax
...
# ---------------------------------------------------------
# 修改: C3k2 整合 Attention Residuals
# ---------------------------------------------------------
class C3k2_AttnRes(C2f):
    """Faster Implementation of CSP Bottleneck with 2 convolutions and Attention Residuals."""
    ...
    def __init__(
        ...
        # 針對深度方向引入 AttnRes
        # 為每一個 Block 初始化一個專屬的 SpatialAttnRes 查詢器
        self.attn_res_layers = nn.ModuleList([SpatialAttnRes(self.c) for _ in range(n)])
    ...
    def forward_split(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass using split() instead of chunk() with Attention Residual aggregation."""
        y = self.cv1(x).split((self.c, self.c), 1)
        y = [y[0], y[1]]
...
# ---------------------------------------------------------
# 修改: SPPF 整合 Attention Residuals
# ---------------------------------------------------------
class SPPF_AttnRes_Pyramid(nn.Module):
    """
    改進版 SPPF (方案一)：使用 SpatialAttnRes 動態聚合不同感受野的金字塔特徵
    """
    def __init__(self, c1: int, c2: int, k: int = 5, n: int = 3, shortcut: bool = False):
        super().__init__()
        c_ = c1 // 2  # hidden channels
        ...
        # 【關鍵修改】因為使用 AttnRes 進行融合，輸出的通道數不會疊加！
        # 原本是 c_ * (n + 1)，現在直接維持 c_ 即可，大幅減少了參數量
        ...
        # 實例化 SpatialAttnRes，用於聚合 n+1 個感受野分支
        self.attn_res = SpatialAttnRes(dim=c_)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        y = [self.cv1(x)]
        
        # 產生連續不同感受野的特徵: [cv1(x), pool1, pool2, pool3]
        y.extend(self.m(y[-1]) for _ in range(getattr(self, "n", 3)))
        
        # 【關鍵修改】使用 Attention Residuals 取代原先的 torch.cat(y, 1)
        # 動態學習這 n+1 個特徵圖的權重並進行融合
        y_fused = self.attn_res(y)
        
        out = self.cv2(y_fused)
        return out + x if getattr(self, "add", False) else out


# ---------------------------------------------------------
# 修改: A2C2f、A2C2f_5、A2C2f_5_RoPEMixed 整合 Attention Residuals
# ---------------------------------------------------------
class A2C2f_AttnRes(nn.Module):  
    """
    A2C2f module with residual enhanced feature extraction using ABlock blocks with area-attention. Also known as R-ELAN
...
class A2C2f_5_AttnRes(nn.Module):  
    """
    A2C2f module utilizing modernized ViT-5 ABlock mechanics with R-ELAN mapping and Attention Residuals.
    """
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class A2C2f_5_RoPEMixed_AttnRes(nn.Module):  
    """
    A2C2f module utilizing modernized ViT-5 ABlock mechanics with R-ELAN mapping and Attention Residuals.
    """
    def __init__(self, c1, c2, n=1, a2=True, area=1, residual=False, mlp_ratio=2.0, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            nn.Sequential(*(ABlock_5_RoPEMixed(c_, num_heads, mlp_ratio, area) for _ in range(2))) if a2 else C3k(c_, c_, 2, shortcut, g) for _ in range(n)
        )
