...
class RepNext(nn.Module):
    def __init__(
        ...
        distillation=False,
        drop_rate=0.0,
        img_size=224,  # 新增 img_size 以符合 SMT 和 dummy forward
        **kwargs,
    ):
        super().__init__()
        ...
        # --- 加入 width_list dummy forward 計算機制 (參考 SMT) ---
        self.width_list = []
        try:
            self.eval()
            # 建立一個測試用的 Dummy Tensor 去通過 Forward 計算中途層的通道數量
    ...
    def forward_features(self, x):
        # 改為回傳多層次特徵圖 List 以免於報錯 AttributeError: 'Tensor' object has no attribute 'insert'
        features = []
        x = self.stem(x)
...
# ==========================================
# 重新建構模型註冊 API 以符合不同大小 SMT 使用方式
# ==========================================

@register_model
def repnext_m0(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(40, 80, 160, 320), depth=(2, 2, 9, 1), img_size=img_size, **kwargs)
    return model

@register_model
def repnext_m1(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(48, 96, 192, 384), depth=(3, 3, 15, 2), img_size=img_size, **kwargs)
    return model

@register_model
def repnext_m2(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(56, 112, 224, 448), depth=(3, 3, 15, 2), img_size=img_size, **kwargs)
    return model

@register_model
def repnext_m3(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(64, 128, 256, 512), depth=(3, 3, 13, 2), img_size=img_size, **kwargs)
    return model

@register_model
def repnext_m4(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(64, 128, 256, 512), depth=(5, 5, 25, 4), img_size=img_size, **kwargs)
    return model

@register_model
def repnext_m5(pretrained=False, img_size=224, **kwargs):
    model = RepNext(embed_dim=(80, 160, 320, 640), depth=(7, 7, 35, 2), img_size=img_size, **kwargs)
    return model


if __name__ == "__main__":
    img_h, img_w = 640, 640
    print("--- Creating RepNext M3 model ---")
    ...
    # 模擬測試通過多層次 Forward 提取特徵，不再返回分類單一 Tensor
    input_tensor = torch.randn(1, 3, img_h, img_w)
    print(f"\n--- Testing RepNext M3 forward pass (Input: {input_tensor.shape}) ---")
