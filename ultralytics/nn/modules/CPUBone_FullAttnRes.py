...
# Added: Full Attention Residuals ONLY for Stage 3 & 4
...
class RMSNorm2d(nn.Module):
    """2D RMSNorm adapted for keys in Full Attention Residuals"""
    ...
    def forward(self, x):
        # x shape can be [B, C, H, W] or [Num_sources, B, C, H, W]
...
class FullAttnResContainer(nn.Module):
    """
    Implements Full Attention Residuals from the paper.
    Each layer selectively aggregates ALL previous layer outputs via learned attention weights.
    """
    def __init__(self, layers: list, channels: int):
        super().__init__()
        self.layers = nn.ModuleList(layers)
        
        # Learnable pseudo-queries (w_l) for each layer, initialized to zero
        self.queries = nn.ParameterList([
            nn.Parameter(torch.zeros(1, channels, 1, 1)) for _ in layers
        ])
        
        # RMSNorm applied on the Keys to prevent large-magnitude outputs from dominating
        self.norm = RMSNorm2d(channels)

    def forward(self, x):
        # v_0 = h_1 (The initial Token Embedding / Stage Input)
        v_list = [x]
        
        for i, layer in enumerate(self.layers):
            # 1. Gather all previous sources [v_0, v_1, ..., v_{l-1}]
            ...
            # 4. Aggregation (Input to current layer l)
            h = torch.sum(attn_weights * V, dim=0) # [B, C, H, W]
            
            # 5. Layer Execution (out = h + f_l(h))
            out = layer(h)
            
            # 6. Extract Delta v_l = f_l(h_l)
            # We subtract h from out because CPUBoneBlock internally does out = h + f_l(h)
            v_l = out - h
            v_list.append(v_l)
            
        # 7. Final output is the sum of all accumulated representations
        return sum(v_list)
...
class CPUBoneBackbone(nn.Module):
    def __init__(self, width_list: list[int], depth_list: list[int], in_channels=3, img_size=224, expand_ratio=4, norm="bn2d", act_func="hswish", bb_convattention=False, bb_convin2=False, fastit=False, huge_model=False, bigit=False, grouping=1, smallk_only_lasts=False, lose_transpose=False, just_unfused=False, use_full_attn_res34=True) -> None:
        ...
        ### STAGE 3-4 - Apply Full Attention Residuals optionally
        for w, d in zip(width_list[3:], depth_list[3:]):
            stage_pre = []
            
            # Downsample Block (Stem for this stage)
            ...
            stage_pre.append(ResidualBlock(block, None))
            curr_channels = w
            ...
            # Check if Full Attention Residuals should be applied to the Stride=1 Blocks
            if self.use_full_attn_res34 and len(sub_blocks) > 0:
                attn_container = FullAttnResContainer(sub_blocks, channels=curr_channels)
                stage_pre.append(attn_container)
                self.stages.append(OpSequential(stage_pre))
            else:
                stage_pre.extend(sub_blocks)
                self.stages.append(OpSequential(stage_pre))
...
if __name__ == '__main__':
    ...
    # 測試含有 Full Attention Residuals 的 B1 模型 (只應用在 Stage 3, Stage 4)
    print("--- Creating CPUBone B1 model with Full Attention Residuals in Stage 3 & 4 ---")
    model = cpubone_b1(use_full_attn_res34=True)
